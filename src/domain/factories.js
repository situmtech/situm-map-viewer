...
import {Building, BuildingGeometry, Floor, Poi, PoiCategories, PoiCategory, Pois, Position, Floors, LatLng, Corners} from "./models"
import { PoiToShow, PoisToShow } from "../components/Map"
...
export  function poiCategoryFactory(data){
    ...
    var _poiCategories = []
    data.forEach(e => _poiCategories.push(new PoiCategory(e.id, e.iconUrl, e.selectedIconUrl)))
...
function poiFactoryFromJson(pois, poiCategories){
    ...
    pois.forEach(e=>{
        ...
        _pois.push(new Poi(e.id, e.name, 
            new Position(e.buildingId, e.position.floorId, e.position.lat, e.position.lng), poiCategory))
    } 
    )
...
function floorFactoryFromJson(floorData){
    var _floors = []
    floorData.forEach(e=>_floors.push(new Floor(e.id, e.level, e.maps.mapUrl)))
...
function cornerFactoryFromCornersAndCenter(corners, center){
    ...
    return new Corners(new LatLng(nw[0], nw[1]), new LatLng(ne[0], ne[1]), new LatLng(se[0], se[1]), 
...
export function buildingFactoryFromJson(buildingData, poiCategoryData){
    ...
    const corners = cornerFactoryFromCornersAndCenter(buildingData.corners, buildingData.location)
    ...
    const geometry = new BuildingGeometry(buildingData.location, corners,
        buildingData.dimensions.width, buildingData.dimensions.length, buildingData.rotation)

    return new Building(buildingData.id, buildingData.name, geometry, 
        floors, pois
        )
...
export function poisToPoisToShow(pois){
    ...
    pois.toArray().forEach(p=>{
        _poisToShow.push(new PoiToShow(p.name, p.position.lat, p.position.lng, p.category.iconUrl))
    })
